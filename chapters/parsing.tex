...
In buona sostanza vogiamo una tabella, che dipende dalla grammatica, che mi dice la corripondenza tra simboli terminali e non terminali mi da la produzione, tale per cui implementare il controllo dell'automa è una banalità, tale per cui se la cima è un terminale ti dico se c'è un match o no, se la cima è un non terminale ti dico se posso fare una predizione o meno. La magia grande è la tabella che deve funzionare, non deve contenere nessuna produzione se sono in un posto in cui non dovrei essere.
...
Per costruire la tabella prendo le produzioni della grammatica, le smonto come lato sinistro e letterina e suffisso e la produco così (se c'è un tentativo di riassegnamento mi avvisa che non è una grammatica che posso trattare):
\begin{lstlisting}
...
Allora una cosa su cui possiamo ragionare è questo, supponiamo di sapere per ogni forma sentenziale qual'è la prima lettere che compare nella derivazione. Mi interessa conoscere data una forma sentenziale i prefissi, di tutte le parole che questa forma sentenziale può produrre quali sono i primi caratteri di queste derivazioni.
...
Si suggerirebbe un approccio ricorsivo, questo per guardare i first, ricordando che questo è perchè non abbiamo i first.

\begin{lstlisting}
  def compute_first(G):
    def recursive_first(X):
\end{lstlisting}
...
Ricorsivamente possiao costruire i first, l'idea è che posso usare una regola che ha sessione a sinistra solo se sul nastro vedo uno di questi:
\begin{table}
...
Riempio mettendo first smontando la forma sentenziale tranne la nullabilità il prefisso e se c'è la nullabilità il suffisso:
\begin{lstlisting}
